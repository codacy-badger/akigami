...
import { Box, Item, Add, Del } from './FilterCheckbox.styled';
      ...
      <Box>
        {merged.map(opt => (
          ...
          >
            <Add
              onMouseEnter={() =>
                this.changeHoverable(opt.value, 'positive', true)
              }
              onMouseLeave={() =>
                this.changeHoverable(opt.value, 'positive', false)
              }
              onClick={() =>
                this.handleChange(
                  opt,
                  opt.active === 'positive' ? false : 'positive',
                )
              }
            >
              {opt.label}
            ...
            {(negative || (!negative && opt.negative)) && (
              <Del
                onMouseEnter={() =>
                  this.changeHoverable(opt.value, 'negative', true)
                }
                onMouseLeave={() =>
                  this.changeHoverable(opt.value, 'negative', false)
                }
                onClick={() =>
                  this.handleChange(
                    opt,
                    opt.active === 'negative' ? false : 'negative',
                  )
                }
              >
                <Icon type="minus" />
