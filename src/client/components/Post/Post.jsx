...
class Post extends Component {
  ...
  renderHeader() {
    ...
    const tooltip = <Tooltip id="post-type">{namespace.text}</Tooltip>;
    return (
      <Header paddings>
        <Avatar
          size={45}
        ...
        />
        <Info>
          <User href={post.user.link}>{post.user.displayName}</User>
          <Date>
            {m(post.createdAt)
              .locale('ru')
              .fromNow()}
    ...
    return (
      <Body paddings>
        ...
        {post.attachments.length > 0 && (
          <Attachments>
            {post.attachments.map((attach, i) => <div key={i}>123</div>)}
          </Attachments>
        )}
        <Actions>
          <ButtonToolbar>
            <Button bsSize="sm" onClick={this.cancelEditPost}>
              Отмена
            </Button>
            <Button bsSize="sm" bsStyle="danger" onClick={this.finishEditPost}>
              Сохранить
            </Button>
          </ButtonToolbar>
        </Actions>
  ...
  render() {
    ...
    return (
      <Element>
        {this.renderHeader()}
        {post.edit ? this.renderEditableBody() : this.renderDefaultBody()}
