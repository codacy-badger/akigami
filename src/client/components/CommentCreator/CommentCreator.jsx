...
class CommentCreator extends Component {
  ...
  renderShortView() {
    ...
    return (
      <Short onClick={this.toFull}>
        <Avatar size={30} src={user.getAvatar} title={user.displayName} />
        <Textarea
          minRows={1}
          maxRows={1}
          readOnly
  ...
  renderFullView() {
    ...
    return (
      <Outside
        isOpen={this.store.collapsed}
        onClickOutside={this.store.handleBlur}
      >
        <Full>
          <Header>
            <Avatar size={30} src={user.getAvatar} title={user.displayName} />
          ...
          <Footer>
            ...
            {this.props.embed && (
              <Action bsStyle="link" bsSize="sm" onClick={this.props.onCancel}>
                Отменить
              </Action>
            )}
            <Action
  ...
  render() {
    return (
      <Creator>
        {this.store.collapsed ? this.renderFullView() : this.renderShortView()}
      </Creator>
    );
  }
}
