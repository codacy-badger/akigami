...
import { Wrapper, Rating, Line } from './FilterRating.styled';
...
class FilterRating extends PureComponent {
  ...
  render() {
    ...
    const tooltipG = (
      <Tooltip id="tooltipG">Без возрастных ограничений</Tooltip>
    );
    const tooltipPG = (
      <Tooltip id="tooltipPG">Рекомендуется присутствие родителей</Tooltip>
    );
    const tooltipPG13 = (
      <Tooltip id="tooltipPG13">Детям до 13 лет просмотр не желателен</Tooltip>
    );
    const tooltipR17 = (
    ...
    const tooltipRPlus = (
      <Tooltip id="tooltipRPlus">Лицам до 17 лет просмотр запрещён</Tooltip>
    );
    const tooltipRx = <Tooltip id="tooltipRx">Хентай</Tooltip>;
    return (
      <Wrapper>
        <Line>
