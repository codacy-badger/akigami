...
class Comment extends Component {
    ...
    return (
      <div>
        ...
        <div className="post-edit-actions">
          <ButtonToolbar>
            <Button bsSize="sm" onClick={this.cancelEditComment}>
              Отмена
            </Button>
            <Button
              bsSize="sm"
  ...
  renderDefaultBody() {
    ...
    const editTooltip = <Tooltip id="edit">Редактировать</Tooltip>;
    const delTooltip = <Tooltip id="delete">Удалить</Tooltip>;
    return (
      <div>
        <Body>
          <User href={comment.user.link}>{comment.user.displayName}</User>
          <Content>
            {comment.deleted ? (
    ...
    return (
      ...
      >
        ...
        {comment.replyObject && (
          <CommentCreator
            post={comment.post}
            onCancel={comment.handleCancelReply}
            embed
            replyObject={comment.replyObject}
          />
        )}
        {comment.children &&
          ...
          <Replies>
            {comment.children.map(reply => (
              <Comment user={user} key={reply.id} comment={reply} replied />
            ))}
          </Replies>
        )}
      </Wrapper>
