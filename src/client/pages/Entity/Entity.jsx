...
class Entity extends Component {
  ...
  renderCharacters = () => (
    <Block
      ...
      padded
      title="Персонажи"
      buttons={<Button bsSize="xs">Все</Button>}
    >
  ...
  renderDescription = () => {
    ...
    return (
      <Block padded bordered>
        <Description>{data.entity.description.russian}</Description>
      </Block>
    );
  };
  renderPoster = () => {
  ...
  renderTrailer = () => {
    ...
    return <Trailer image={trailer.thumbnail} url={trailer.url} />;
  };
  renderInfo = () => {
    const { data } = this.props;
    return (
          ...
          <ItemInfo>
            ...
            <span>
              {(data.entity.title.other || []).map((e, i) => (
                <span>
                  {e}
                  {i < data.entity.title.other.length - 1 ? '; ' : '.'}
                </span>
              ))}
            </span>
          </ItemInfo>
        ...
        <Content>
          <Grid>
            ...
            <CenterColumn>
              ...
              <Responsive minWidth={768}>{this.renderDescription()}</Responsive>
              {this.renderRating()}
              <Responsive maxWidth={1199}>
                <Grid>
                  <RightColumn>{this.renderCharacters()}</RightColumn>
                  <RightColumn>{this.renderStaff()}</RightColumn>
                </Grid>
              </Responsive>
              ...
              <Responsive maxWidth={1199}>
                <Grid>
                  <RightColumn>{this.renderTracks()}</RightColumn>
                  <RightColumn>{this.renderFriends()}</RightColumn>
                </Grid>
                {this.renderClubs()}
              </Responsive>
              {this.renderReviews()}
